...
const Article = ({ title, point1,point2,point3,point4,point5 ,summary, time, img, closeArticle }) => {
  return (
    <div className="col-span-2 w-full p-4 bg-light relative border border-solid border-dark rounded-2xl dark:bg-dark dark:border-light">
      ...
      <span className="text-primary font-semibold dark:text-orange-500">{time}</span>
      <button onClick={closeArticle} className="w-full absolute bottom-4 right-4 text-dark hover:text-primary dark:text-light dark:hover:text-orange-500">
        Close
      </button>
    </div>
  );
